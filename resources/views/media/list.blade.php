    ...
    <div class="modal fade" id="modal-edit" aria-hidden="true" style="display: none;">
        <div class="modal-dialog">
            ...
            <form id="form1" class="tdiojax-form"
                  action="{{URL::to("post/mediaEdit")}}"
                  method="post"
                  data-error-container="error_container"
                  data-after-success-redirect="{{ URL::to("/media/list") }}"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group row">
                            ...
                            <p class="col-md-9">
                                <select class="form-control" name="file_cat">
                                    @if($data->categories && count($data->categories)>0)
                                        <option value="">Select</option>
                                        @foreach($data->categories as $c)
                                            <option value="{{ $c->id }}">{{ ucwords($c->cat_name) }}</option>
                                        @endforeach
                                    @else
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function()
        {
            ...
            $('body').on('click','.btn-edit',function()
            {
                ...
                if(data!=null)
                {
                    $('input[name="id"]').val(data[_ind].id);
                    $('input[name="file_name"]').val(data[_ind].file_name);
                    $('select[name="file_cat"]').val(data[_ind].file_cat);
                }
            });
        ...
        function getDataTableData()
        {
            ...
            ({
                ...
                dom: 'Bfrtip',
                buttons: [],
                ajax: '{{ URL::to('api/mediaList')  }}',
                initComplete: function(prop, _data)
                {
                    data = _data.json;
                    $('#global_loader').hide();
